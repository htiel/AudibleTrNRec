...
function buildFeasibilityCard(rows) {
  ...
  const note = uncertain.length > 0 ? h('p', { class: 'lcars-note', role: 'status', text: `Uncertainty against the bundled synthetic fixture catalog only: ${uncertain.map((row) => `${row.label} is unknown for ${row.unknownCount} of ${row.total} titles`).join('; ')}.` }) : h('p', { class: 'lcars-note', text: 'Every measured field is fully known across the bundled synthetic fixture catalog.' });
  return h('div', {}, [table, note]);
}

function buildTrace(trace) {
  const { edges, shown, total, limit, truncated } = trace;
  if (edges.length === 0) return h('div', {}, [h('p', { text: 'No shared-facet edges are available in the bundled synthetic fixture catalog.' }), h('p', { class: 'lcars-note', text: `Showing 0 of ${total} shared-facet edges (display limit ${limit}).` })]);
  return h('div', {}, [
    h('p', { class: 'lcars-note', role: 'status', text: truncated ? `Showing ${shown} of ${total} shared-facet edges from the bundled synthetic fixture catalog. The list is capped at ${limit} edges, so this listing is incomplete.` : `Showing all ${shown} of ${total} shared-facet edges from the bundled synthetic fixture catalog (display limit ${limit}).` }),
    h('ul', { class: 'lcars-trace-list' }, edges.map((edge) => h('li', { class: 'lcars-trace-edge' }, [h('p', { class: 'lcars-trace-heading', text: `Shared ${formatFacetKind(edge.kind).toLowerCase()}: ${formatText(edge.name)} (${edge.titles.length} titles)` }), h('ul', {}, edge.titles.map((title) => h('li', { text: formatText(title) })))]))),
  ]);
}
