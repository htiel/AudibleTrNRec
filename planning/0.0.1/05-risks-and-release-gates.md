...
**Current disposition:** Locked, unanimously approved alpha 0.0.1 baseline.
Every gate remains pending. Original reviews are conditional/withheld, not final
approval of this revision. No access, legal, security, architecture, accessibility
or release approval has been obtained by creating these documents.
...
| R06 Incomplete deletion | Personal data survives in caches/backups/exports | Medium / High | 9 | 4 | 5 | 180 | One encrypted container/key destruction, expanded inventory, restart/canary re-scan, hard retention, user-copy/SSD limits | Worf | G2/G5 |
...
R14–R17 have no residual acceptance yet. At G2 relevant safety risks with RPN
>=100 require executed mitigation/re-score <100, not deferral to G5; at G5 every
risk must be below 100 and have no open security/a11y blocker regardless of score.
...
| G1 — Approved experiment route/design | S001/S002/S003 complete; all G1 conditions below; named permission authority, disqualified routes, consent/incident/archive design, neutral contract and numeric caps | Worf + Data; Geordi approves consent/surface design; Wesley semantic/trust design; Captain approves fallback and region/language | No real-data/source/catalog activity; ambiguous permission escalated to qualified authority |
...
| 3 | Static credential absence: no password/MFA/session-cookie, browser profile, embedded webview or automation code/dependency | Data S013/S009; Worf boundary review | G2 |
...
| 9 | Key destruction, full storage/residue inventory, restart and canary re-scan find no recoverable app-managed data; limitations disclosed | Data S009; Worf witnesses | G2 |
...
| 11 | Cancel/denied/malformed/interrupted/timeout faults actionable and contain no private diagnostic category | Data S013/S009 | G2 |
...
| 13 | Actual adapter per-run request/byte/runtime caps enforce abort, not silent truncation; safe fixture fault validation plus bounded actual-run counts | Data S004/S005 | G3 |
...
| --- | --- | --- |
| D01 Access route | TBD; preference order official documented authorization/API, then supported export | S001/S004, Data + Worf, G1 approval / G3 proof |
